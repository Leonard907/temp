...
class LongT5LocalAttention(nn.Module):
    ...
    def forward(
        ...
        if position_bias is None:
            ...
            if mask is not None:
                # Replace masked positions with torch.finfo(torch.float16).min (according to the original implementation)
                mask = torch.where(mask > 0, 0.0, torch.finfo(torch.float16).min)
...
class LongT5TransientGlobalAttention(nn.Module):
    ...
    def compute_side_bias(self, mask: torch.Tensor, global_segment_ids: torch.Tensor) -> torch.Tensor:
        ...
        attention_side_bias = torch.where(side_attention_mask > 0, 0.0, torch.finfo(torch.float16).min)
    ...
    def forward(
        ...
        if mask is not None:
            ...
            local_attention_mask = torch.where(local_attention_mask > 0, 0.0, torch.finfo(torch.float16).min)
        else:
...
class LongT5Block(nn.Module):
    ...
    def forward(
        ...
        # logger.info(hidden_states.dtype)
